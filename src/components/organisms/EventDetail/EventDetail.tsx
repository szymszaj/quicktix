      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-3">
        <div className="lg:col-span-2 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <InfoTile Icon={Calendar} label="Data" value={formatDate(event.date)} />
            <InfoTile Icon={Clock} label="Godzina" value={event.time} />
            <InfoTile Icon={MapPin} label="Miejsce" value={`${event.venue}, ${event.city}`} />
            <InfoTile
              Icon={Armchair}
              label="Wolne miejsca"
              value={event.availableSeats === 0 ? "Wyprzedane" : event.availableSeats.toLocaleString("pl-PL")}
            />
          </div>

          <div>
            <h2 className="mb-3 text-lg font-semibold text-gray-900">O wydarzeniu</h2>
        ...
        <aside className="flex flex-col gap-4">
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <p className="mb-1 text-sm text-gray-400">Cena za bilet</p>
...
const InfoTile = ({ Icon, label, value }: { Icon: React.ElementType; label: string; value: string }) => (
  <div className="rounded-xl border border-gray-100 bg-white p-4">
    <Icon size={18} className="mb-2 text-gray-400" strokeWidth={1.75} />
